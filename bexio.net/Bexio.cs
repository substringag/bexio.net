...
namespace bexio.net
{
	public class Bexio
	{
		...
		public List<Project> GetProjects(string orderBy = "name", int offset = 0, int limit = 1000)
		{
		...
		public Timesheet CreateTimesheet(TimesheetBase data)
		{
		...
		public List<TimesheetStatus> GetTimesheetStatus(string orderBy = "name", int offset = 0, int limit = 1000)
		{
			var request = new RestRequest($"2.0/timesheet_status?order_by={orderBy}&offset={offset}&limit={limit}", Method.GET);
			IRestResponse response = ExecuteRestRequestWithBearer(request);
			List<TimesheetStatus> list = Newtonsoft.Json.JsonConvert.DeserializeObject<List<TimesheetStatus>>(response.Content);
			return list;
		}
		#endregion
		...
		public List<BusinessActivity> GetBusinessActivities(string orderBy = "name", int offset = 0, int limit = 1000)
		{
